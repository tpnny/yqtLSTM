...
with torch.no_grad():
    for i, (input, target) in enumerate(test_loader):
        ...
        model.batch_size = input_.shape[0]
        model.hidden_cell = (
            torch.zeros(model.num_layers * model.num_directions, model.batch_size, model.hidden_size).to(device),
            torch.zeros(model.num_layers * model.num_directions, model.batch_size, model.hidden_size).to(device))
